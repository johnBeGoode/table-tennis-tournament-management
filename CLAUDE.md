...
4. **`BarrageScreen.jsx`** — matchs de barrage entre 3es, id `barrage-{poolIdA}-{poolIdB}` (A = le mieux classé des deux par `crossPoolCompare`). La règle vit dans `buildPrincipalSeeds` (`AppShell.jsx`) mais est **recopiée** dans `BarrageScreen.jsx`, `ConsolanteScreen.jsx` et `BracketsScreen.jsx` : toute modification doit être répercutée aux quatre endroits. Le résultat stocke des **copies** des joueurs (`p1`/`p2`) ; c'est `pruneBarrageResults` qui garantit qu'elles correspondent encore aux 3es courants.
...
6. **`ConsolanteScreen.jsx`** — même classement intégral que le principal (`buildIntegralBracket(seeds, 'consolante', …, { byes: true })`), mêmes colonnes et encadrés, plus un placement manuel par drag & drop. La différence : la consolante n'est pas forcément pleine (taille = puissance de 2 supérieure au nombre d'éligibles), donc un slot vide du 1er tour est un **bye structurel** qui se propage — l'adversaire passe d'office, bye contre bye reste un bye, et les places du bas restent vides. Dans le principal, un slot vide est un barrage en attente et n'avance pas (option `byes` absente). Le bouton « Auto » applique `window.buildSeedingPattern` — le **même** placement que le tableau principal.
