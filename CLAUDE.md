...
Le mode dépend du **nombre de poules**, pas du nombre de joueurs (`computeBracketStructure(2n, n)`). Le tableau fait toujours la puissance de 2 **supérieure** à 2n ; tous les 1ers et 2es sont qualifiés. **Il n'y a jamais de barrage** : les places manquantes vont aux meilleurs 3es s'il y en a assez, sinon elles restent vides (exemptés) :

| Poules | Tableau | Mode | Remarque |
|---|---|---|---|
| 2, 4, 8, 16 | 4, 8, 16, 32 | `direct` | 2n est déjà une puissance de 2 |
| 3, 6, 7, 11, 12, 13, 14, 15 | 8, 16, 32 | `thirds` | `B − 2n` ≤ nombre de 3es : les `B − 2n` **meilleurs 3es** (`crossPoolCompare`, quotients inter-poules) complètent le tableau, TS `2n+1..B` dans l'ordre des poules ; les autres 3es et les 4es en consolante |
| 5, 9, 10 | 16, 32 | `byes` | pas assez de 3es : les seeds `2n+1..B` du pattern restent vides, `B − 2n` TS (celles dont le voisin de position est vide, pas « les meilleures ») sont **exemptées de 1er tour** ; 3es et 4es en consolante |

Il n'y a **plus de mode `eliminate`** (aucun 2e n'est écarté) **ni de mode `barrage`**. Pour voir des meilleurs 3es : **28 joueurs** (7 poules → 2 meilleurs 3es) ou **12 joueurs** (3 poules → tableau de 8, 2 meilleurs 3es) ; pour voir des exemptés dans le principal : **20 joueurs** (5 poules → tableau de 16, 6 exemptés = les 5 1ers + le meilleur 2e).
...
`ConsolanteScreen` ne place personne automatiquement : le bracket est celui qu'on a construit à la main (drag & drop) ou via le bouton « Auto », et il est rechargé tel quel depuis `consolante-seeds-v7` à chaque montage. **Changer `buildSeedingPattern` ou la numérotation des TS n'a donc aucun effet visible** tant qu'on n'a pas recliqué sur « Auto » : l'ancien placement survit et donne l'impression que la correction n'est pas passée. Le `useEffect` de resynchronisation ne remet à zéro que si la **taille** du bracket change, pas si le pattern change.
...
| `ertt-bracket-schema` | version du schéma des résultats de tableau (`2` = mode `byes` à la place d'`eliminate`, `3` = nouvelles tables 8/16/32 et 2es placés en moitié opposée de leur 1er, `4` = plus de barrages, mode `thirds`). Au passage vers 3, `App` vide `ertt-bracket-results` **une fois** dans tous les modes (les ids `principal-r1-N` ne désignent plus les mêmes joueurs) ; au passage 3→4, purge **une fois** si le tournoi courant est en mode `thirds`, et suppression de l'ancienne clé `ertt-barrage-results` |
...
- `computeBracketStructure(autoQualifiers, thirdsCount)` → `{ bracketSize, mode: 'direct' | 'thirds' | 'byes', thirdsQualified, byeCount }`. `bracketSize` est toujours la puissance de 2 supérieure ; décide si les places manquantes sont comblées par les meilleurs 3es (assez de 3es) ou laissées vides comme exemptions de 1er tour.
...
- Après un « Réinitialiser », la clé `consolante-seeds-v7` **réapparaît avec la valeur `[null]`** dès qu'on visite l'écran Consolante : le `useEffect` de persistance réécrit l'état vide (bracket de taille 1). Sans conséquence, mais ne pas y voir un échec de `clearConsolanteSeeds()` — regarder le contenu, pas l'existence de la clé.
