...
de Classements / Tab principal, badges d'Informations). Dans le code, la variable s'appelle
encore `seed` ; ne la présente jamais comme une « tête de série » dans l'interface.

## Format des matchs

Le choix « 2 ou 3 sets gagnants » (écran Poules, clé `ertt-sets-to-win`) ne vaut que pour
les **matchs de poule**. Les **deux tableaux — principal ET consolante — se jouent toujours en 3 sets gagnants** (best of 5) :
`SETS_TO_WIN` est donc codé en dur à `3` dans `KnockoutScreen.jsx` et dans
`ConsolanteScreen.jsx`. **Ce n'est pas un oubli de câblage** — ne pas « corriger » ces
constantes en y branchant `setsToWin`. Le libellé de l'écran Poules dit bien « Format des
matchs de poule », et le verrou de ce choix (dès qu'une clé `pool-*` existe dans `results`)
ne concerne lui aussi que les poules.
...
| `ertt-sets-to-win` | `2` ou `3` (défaut `3`) — format des **matchs de poule uniquement**. Les tableaux ignorent ce réglage, cf. « Format des matchs » ci-dessous |
...
- `parsePlayersCsv(text)` → `{ players: [{ name, ranking, line }], ignored: [{ line, raw }], error }`. Lecture du CSV d'import (bouton « Importer (.csv) », écran Poules). Le fichier attendu porte **deux données et deux seulement : le nom (nom, prénom ou les deux) et le classement** — ni licence, ni club, ni catégorie. La mise en forme reste tolérante : séparateur deviné (`;`, `,`, tabulation), en-tête optionnel ignoré, guillemets RFC 4180, BOM, CRLF, espaces et séparateurs en trop en fin de ligne. Lecture **par position** : la **dernière** cellule est le classement dès qu'elle en a l'air (nombre, marqueur `NC` / `non classé` / `loisir`, ou rien qui contienne une lettre), tout le reste forme le nom — une dernière cellule qui est visiblement un mot est donc **rattachée au nom** (« Dupont;Jean » → « Dupont Jean » à 500) plutôt que perdue. Classement absent, illisible ou hors de portée (> `CSV_MAX_RANKING`, 9999) : plancher à 500 via `normalizeRanking`, jamais d'erreur — la valeur retenue est montrée dans l'aperçu avant import. **N'attribue aucun id** : c'est `PoolsScreen` qui le fait (un seul `Date.now()`, jamais sous le plus grand id existant). Les lignes sans nom (aucune lettre) partent dans `ignored`, affichées dans l'aperçu. Enfin, `error` (`'rtf'`, `'zip'`, `'pdf'`, sinon `null`) signale un fichier **qui n'est pas du texte** malgré son extension — le cas courant étant le document TextEdit resté en texte enrichi, que macOS enregistre en RTF même nommé `.csv` : sans ce garde-fou la lecture « réussit » et crée des joueurs nommés `{\rtf1\ansi…`. La liste est alors vide, à `PoolsScreen` d'expliquer quoi faire.
...
1. **`PoolsScreen.jsx`** — saisie des joueurs, **import CSV**, choix du format (2 ou 3 sets gagnants), constitution des poules. Le bouton « Importer (.csv) » (à côté de « Joueurs (N) », masqué comme la saisie manuelle quand les poules sont verrouillées) ouvre un fichier, le lit en UTF-8 avec **repli Windows-1252** si le décodage échoue (CSV des Excel français), le passe à `window.parsePlayersCsv` et affiche un **aperçu** : rien n'est ajouté avant confirmation. Les joueurs importés **s'ajoutent** à la liste ; les doublons de nom (casse, accents et espaces ignorés — dans le fichier comme avec les joueurs déjà saisis) et les lignes sans nom sont écartés mais **listés** dans l'aperçu. Il n'y a **plus de création manuelle de poule** : les poules ne naissent que de la répartition auto (le retrait/ajout d'un joueur dans une poule existante reste possible une fois déverrouillé). La **répartition auto** applique la méthode du serpent (`snakeDistribute`) sur les joueurs triés du meilleur au moins bon, **remplace intégralement** les poules existantes et **pose le verrou** `poolsLocked` (état d'App, clé `ertt-pools-locked`) : toutes les actions de modification disparaissent de l'écran jusqu'à « Déverrouiller ». Le choix du format se **verrouille** dès qu'une clé `pool-*` existe dans `results` — c'est un verrou distinct.
